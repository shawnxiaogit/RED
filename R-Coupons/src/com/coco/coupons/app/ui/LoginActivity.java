...
//				String pwd = login_password_edit.getText().toString();
				...
				String userName = login_account_edit.getText().toString();
				
				
				account = userName;
				String pwd = login_password_edit.getText().toString();
				
				if (!checkNamepwdIsNull(userName, pwd)) {
					...
					// 1、新增本地账号匹配
//					if (accountAndPwdCorrect(userName, pwd)) {
						// 保存用户名为一个临时全局变量
...
//						String mac_key = "3133363739363035";
						MyApplication.PHONE_NUM = userName;
...
//						String card_num = MyApplication.PHONE_NUM_ENCRYPT+"00000";
						...
						mRunningTask = new LoginTask(userName, pwd, MyApplication.IMSI, MyApplication.IEMI,
								isoMsg);
...
//						MyApplication.CURRENT_CHAXUN_HAO = node.getChaxunhao();
						 MyApplication.CURRENT_CHAXUN_HAO = "000000";
...
//							L.e("MyApplication.myMepAuority", ""+MyApplication.myMepAuority);
						...
						openActivity(MainMenuActivity.class);
						MyApplication.switchActivityAnimationOut(LoginActivity.this);
						defaultFinish();
					} else {
