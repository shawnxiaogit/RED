...
//							System.out.println("61-len:" + len);
							...
							if(len>0){
								...
								for(int m = 0;m<datas.length;m++){
									System.out.println("datas["+m+"]:"+datas[m]);
									String myData = datas[m];
									if(m==0){
										node.setmCustPhoneNum(myData);
									}
									if(m==1){
										node.setmCustNum(myData);
									}
									if(m==2){
										node.setmCustName(myData);
									}
								}
...
//		String response = "30303734081020380000028000193939323030303032343130373032313233303130333030303030303030303032303134323031333130333030323132333030303432373134B5169DA81BB19841";
		String response = "";
		parseMsg(response);
	}
