...
//		int t_sum = (i_sum*100);
		Double double1 = Double.parseDouble(sum);
...
//		int t_sum = (i_sum*100);
	 ...
	 * @return
	 */
	public static String getDataLen2(BaseNode isoMsg){
		StringBuilder sb2 = new StringBuilder();
		String hex_data = isoMsg.getHexString2();
		int len2 = hex_data.length();
		int rea_len = (len2/2);
		if(rea_len>99&&rea_len<999){
			sb2.append("0");
...
//		L.i("hex_data", ""+hex_data.length());
	 ...
	 * @return
	 */
	public static int getDataSize2(BaseNode isoMsg){
		int result = 0;
//		L.i("getDataIntLen", "getDataIntLen()");
		StringBuilder sb2 = new StringBuilder();
		String hex_data = isoMsg.getHexString2();
...
//		sb.append(new String(b_len));
	 ...
	 * @return
	 */
	public static byte[] getMiWenData3(byte[] len3,byte[] datas){
		L.e("getMiWenData", "getMiWenData3");
		int len = Integer.parseInt(new String(len3));
//		int len = datas.length;
//		String str_len = String.valueOf(datas.length);
		L.e("len:", ""+len);
		byte[] one_zeor= "0".getBytes();
...
//		StringBuilder sb = new StringBuilder();
	 ...
	 * @return
	 */
	public static byte[] getMiWenData2(int real_len,byte[] datas){
		L.e("getMiWenData", "getMiWenData");
		int len = datas.length;
		String str_len = String.valueOf(real_len);
		L.e("len:", ""+len);
		byte[] one_zeor= "0".getBytes();
		byte[] two_zeros = "00".getBytes();
		if(len>0&&len<=99){
	 ...
	 * @return
	 */
	public static String printRequest2(BaseNode isoMsg){
		StringBuilder sb = new StringBuilder();
		String str_len=DigitalTrans.stringToHexString(getDataLen(isoMsg));
		sb.append(str_len);
		sb.append(isoMsg.getHexString2());
		return sb.toString();
	}
	/**
	 * 获取请求报文的字节数组
...
//			}
	...
	public static byte[] getBytesRequest2(BaseNode isoMsg){
		int dataSize = RequestUtil.getDataSize2(isoMsg);
//		System.out.println("dataSize:"+dataSize);
		byte[] request = new byte[dataSize*2];
		int count =0;
		String header=RequestUtil.getDataLen2(isoMsg);
		for(int i=0;i<header.getBytes().length;i++){
			request[count]=header.getBytes()[i];
			count++;
		}
...
//			}
	 ...
	 * @return
	 */
	public static byte[] getMacBytes2(BaseNode isoMsg){
		int dataSize = RequestUtil.getDataSize(isoMsg);
		System.out.println("dataSize:"+dataSize);
		byte[] request = new byte[dataSize*2];
		int count =0;
	 ...
	 * @return
	 */
	public static int getMacLen2(BaseNode isoMsg){
		int count =0;
		String MTI = isoMsg.getMTI();
		if(MTI!=null&&MTI.length()>0){
			for(int i=0;i<MTI.getBytes().length;i++){
